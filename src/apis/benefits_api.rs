...
/// struct for typed errors of method [`benefits_retrieve`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum BenefitsRetrieveError {
    UnknownValue(serde_json::Value),
}


/// Returns a list of `Benefit` objects.
pub async fn benefits_list(configuration: &configuration::Configuration, x_account_token: &str, created_after: Option<String>, created_before: Option<String>, cursor: Option<&str>, employee_id: Option<&str>, include_deleted_data: Option<bool>, include_remote_data: Option<bool>, modified_after: Option<String>, modified_before: Option<String>, page_size: Option<i32>, remote_id: Option<&str>) -> Result<crate::models::PaginatedBenefitList, Error<BenefitsListError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/benefits", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<BenefitsListError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn benefits_retrieve(configuration: &configuration::Configuration, x_account_token: &str, id: &str, include_remote_data: Option<bool>) -> Result<crate::models::Benefit, Error<BenefitsRetrieveError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/benefits/{id}", local_var_configuration.base_path, id=crate::apis::urlencode(id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<BenefitsRetrieveError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
