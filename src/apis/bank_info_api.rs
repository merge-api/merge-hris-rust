...
/// struct for typed errors of method [`bank_info_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum BankInfoListError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`bank_info_retrieve`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum BankInfoRetrieveError {
    UnknownValue(serde_json::Value),
}


/// Returns a list of `BankInfo` objects.
pub async fn bank_info_list(configuration: &configuration::Configuration, x_account_token: &str, account_type: Option<&str>, bank_name: Option<&str>, created_after: Option<String>, created_before: Option<String>, cursor: Option<&str>, employee_id: Option<&str>, include_deleted_data: Option<bool>, include_remote_data: Option<bool>, modified_after: Option<String>, modified_before: Option<String>, order_by: Option<&str>, page_size: Option<i32>, remote_fields: Option<&str>, remote_id: Option<&str>) -> Result<crate::models::PaginatedBankInfoList, Error<BankInfoListError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/bank-info", local_var_configuration.base_path);
    ...
    if let Some(ref local_var_str) = bank_name {
        local_var_req_builder = local_var_req_builder.query(&[("bank_name", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<BankInfoListError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn bank_info_retrieve(configuration: &configuration::Configuration, x_account_token: &str, id: &str, include_remote_data: Option<bool>, remote_fields: Option<&str>) -> Result<crate::models::BankInfo, Error<BankInfoRetrieveError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/bank-info/{id}", local_var_configuration.base_path, id=crate::apis::urlencode(id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<BankInfoRetrieveError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
